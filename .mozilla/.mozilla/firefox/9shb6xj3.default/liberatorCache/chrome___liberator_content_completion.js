        ...
        for (let [key, context] in Iterator(context.contexts)) {
            if (key.startsWith("/list")) {
                let list = template.genericOutput("",
                    xml({raw: ["<div highlight=\"Completions\">\n\
                        ", "\n\
                        ", "\n\
                    </div>"],cooked: ["<div highlight=\"Completions\">                        ", "                        ", "                    </div>"]}, [( template.completionRow(context.title, "CompTitle") ), ( template.map2(xml, context.items, function (item) context.createRow(item), null, 100) )]));
